...
static void set_layer_status(struct zmk_widget_status *widget, struct layer_status_state state) {
    ...
    draw_layer(widget->obj, widget->cbuf2, &widget->state);
}
...
static void set_caps_word_status(struct zmk_widget_status *widget,
                                 struct zmk_caps_word_state_changed state) {
    widget->state.caps_word_active = state.active;
    lv_obj_set_style_bg_color(widget->obj, lv_color_white(), LV_PART_MAIN);
    lv_obj_set_style_bg_opa(widget->obj, state.active ? LV_OPA_COVER : LV_OPA_TRANSP,
                            LV_PART_MAIN);
    draw_top(widget->obj, widget->cbuf, &widget->state);
    draw_layer(widget->obj, widget->cbuf2, &widget->state);
}

static void caps_word_status_update_cb(struct zmk_caps_word_state_changed state) {
    struct zmk_widget_status *widget;
    SYS_SLIST_FOR_EACH_CONTAINER(&widgets, widget, node) { set_caps_word_status(widget, state); }
}

static struct zmk_caps_word_state_changed caps_word_status_get_state(const zmk_event_t *eh) {
    const struct zmk_caps_word_state_changed *ev = as_zmk_caps_word_state_changed(eh);
    return (struct zmk_caps_word_state_changed){.active = (ev != NULL) ? ev->active : false};
}

ZMK_DISPLAY_WIDGET_LISTENER(widget_caps_word_status, struct zmk_caps_word_state_changed,
...
int zmk_widget_status_init(struct zmk_widget_status *widget, lv_obj_t *parent) {
    ...
    lv_obj_align(layer, LV_ALIGN_TOP_LEFT, 24, 0);
    lv_canvas_set_buffer(layer, widget->cbuf2, CANVAS_SIZE, CANVAS_SIZE, LV_IMG_CF_TRUE_COLOR);
