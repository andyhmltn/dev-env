...
YOU MUST ensure all prettier / tsc checks pass before a change is finished. For a typical pnpm project this is:

```
pnpm run check:lint && pnpm run check:tsc && pnpm run check:prettier
```

Some projects need auth tokens exported before `pnpm install` will resolve private packages. Export them in the same shell invocation as the install when required.

# Worktrees

Always create git worktrees in `<repo-root>/.worktrees/<name>`, never anywhere else. If `.worktrees/` is not gitignored, add it to `.git/info/exclude` rather than committing a .gitignore change.
