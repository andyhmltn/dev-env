...
- **Caps word** (`ht_caps_word`): hold `D`. Uses the same balanced flavor and right-hand trigger positions as `F`, while tapping sends `D`. Auto-capitalizes until a non-alpha key
